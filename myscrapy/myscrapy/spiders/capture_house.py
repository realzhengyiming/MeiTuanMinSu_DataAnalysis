...
class HotwordspiderSpider(scrapy.Spider):
    ...
    custom_settings = {  # 每个爬虫使用各自的自定义的设置
        ...
        'DOWNLOAD_TIMEOUT': 60,
        'DOWNLOAD_DELAY': 3,  # 慢慢爬呗  ,这里写了下载延迟， 403就IP被封了 todo
    ...
    def start_requests(self):
        ...
        import requests
        print("进来了")
        raw = requests.get("http://127.0.0.1/hotelapp/get_fav_house_by_id/?api=asdsewrzt!dfe")  # 随便了
        jsonString = json.loads(raw.content)
        print(jsonString)
        if jsonString['data']['result'] == []:  # 空对结果数组
            print("finish 监控为空")
            pass
        else:
            for house_id in jsonString['data']['result']:
                print(house_id[0])
                print(f"https://minsu.meituan.com/housing/{house_id[0]}/")
                yield scrapy.Request(url=f"https://minsu.meituan.com/housing/{house_id[0]}/",
                                     dont_filter=True,
